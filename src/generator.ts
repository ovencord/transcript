...
import { css, htmlTemplate } from './template.ts';
import type {
	ActionRow,
	AnyComponent,
	Button,
...
function formatContent(content: string, userMap?: Map<string, string>): string {
	...
	html = html.replace(/&lt;t:(\d+):?([A-Z])?&gt;/g, (_match, timestamp, _style) => {
...
export async function generateTranscript(messages: Message[], channel: ChannelInfo, options: TranscriptOptions = {}) {
	...
	for (const msg of messages) {
		if (msg.author?.id && msg.author.username) {
	...
	// biome-ignore lint/suspicious/noExplicitAny: mustache module requires any cast for render()
